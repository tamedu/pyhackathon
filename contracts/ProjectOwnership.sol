...
contract ProjectOwnership {
    ...
    returns (bool) {
        ...
        if (getParticipantId(msg.sender, t.approvedParticipants) > -1) {
            ...
            checkTodoApprovement(t);
            return true;
        }
    }

    function getTodoApprovedCount(uint todoIndex)
    public view
    onlyParticipants()
    returns (uint) {
        return todos[todoIndex].approvedParticipants.length;
    }
    ...
    function checkTodoApprovement(Todo t)
    private
    returns (bool) {
        uint256 i = 0;
        uint256 id;
